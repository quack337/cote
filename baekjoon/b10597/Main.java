package baekjoon.b10597;
import java.io.*;
import java.util.*;

public class Main {
  static String A;  // 입력으로 주어진 문자열
  static int N;
  static List<Integer> S = new ArrayList<Integer>(); // 지금까지 선택한 수 목록
  static boolean[] V; // 숫자 i를 선택했는지 표시하기 위한 배열. 중복 선택을 막기 위함
  static String 답;

  public static void main(String[] args) throws IOException {
    var br = new BufferedReader(new InputStreamReader(System.in));
    A = br.readLine().trim();  // 입력으로 주어진 문자열
    N = 1; // 순열의 크기. 아직 순열의 크기를 모르기 때문에 1로 초기화함.
    for (int n=0; ; ++N) { // ++N 하면서 순열의 크기를 찾는다
    ...
    V = new boolean[N+1];  // 숫자 i를 선택했는지 표시하기 위한 배열. 중복 선택을 막기 위함
    DFS(0);
    System.out.println(답);
  }

  static boolean DFS(int i) { // i: A 문자열에서의 현재 위치 인덱스
                  // 지금까지 선택한 S 순열은, A 문자열에서 인덱스 i 직전까지 일치한다
    if (i >= A.length()) { // 문자열 끝까지 일치했다면,  S 순열이 답이다
      // S 순열을 문자열로 만들어서 답에 대입한다
    ...
    for (int j = i + 1; j <= A.length(); ++j) {
      // 인덱스 i 부터 j 직전까지의 부분문자열을 정수 x로 변환
      int x = Integer.parseInt(A.substring(i, j));
