...
public class Main {
    ...
    static int 탐색(int 세로선_수, int 가로위치_수, boolean[][] 가로선, int 인덱스, int 추가한_가로선수) {
        ...
        가로선[세로][가로] = true; // 가로선을 추가함.
        if (확인(세로선_수, 가로위치_수, 가로선)) {
            가로선[세로][가로] = false; // 리턴하기 전에, 추가했던 가로선을 제거함.
            return Math.min(결과1, 추가한_가로선수 + 1);
        }
        int 결과2 = 탐색(세로선_수, 가로위치_수, 가로선, 인덱스 + 1, 추가한_가로선수 + 1);
        가로선[세로][가로] = false;     // 리턴하기 전에, 추가했던 가로선을 제거함.
