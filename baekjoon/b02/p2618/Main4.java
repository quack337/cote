...
public class Main4 {
    ...
    static String sol2() {
        ...
        while (true) {
            int index = Math.max(index1, index2) + 1;
            if (index == W) return builder.toString();
            int r = A[index][0], c = A[index][1];
            int r1 = 1, c1 = 1, r2 = N, c2 = N;
            if (index1 >= 0) {
                r1 = A[index1][0];
            ...
            int distance1 = distance(r, c, r1, c1) + DP[index + 1][index2 + 1];
            int distance2 = distance(r, c, r2, c2) + DP[index1 + 1][index + 1];
