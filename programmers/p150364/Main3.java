    ...
    static class Solution {
        ...
        void drop(int node, int dropNo) {
            if (isLeaf[node]) {
                ...
                if (isValidLeaf[node]) {
                    isValidLeaf[node] = false;
                    --validLeafCount;
                }
                if (DP[drops[node].size()][target[node]] != null) {
                    isValidLeaf[node] = true;
                    ++validLeafCount;
                }
            } else {
        ...
        public int[] solution(int[][] edges, int[] target) {
            ...
            for (int d = 0; d < sum; ++d) {
                ...
                if (validLeafCount == leafCount) {
                    System.out.printf("성공 leaf:%d drop:%d\n", leafCount, d+1);
                    for (int i = 0; i < N; ++i)
            ...
            if (validLeafCount != leafCount)
                System.out.printf("실패 leaf:%d validLeaf:%d\n", leafCount, validLeafCount);
            return null;
        }
    }
