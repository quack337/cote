...
public class Test2 {
        ...
        public int solution(int[][] board) {
            ...
            for (int r = 0; r < board.length; ++r)
                for (int c = 0; c < board.length * 2; ++c) {
                    int c1 = (c < board.length) ? c : board.length * 2 - c - 1;
                    for (int[] size : SIZES)
                        if (채울수있나(board, r, c1, size[0], size[1])) {
                            ++count;
                            제거(board, r, c1, size[0], size[1]);
                        }
                }
            return count;
        }
    ...
    public static void main(String[] args) {
        ...
        out.println(s.solution(new int[][] {{1,1,1},{0,1,0},{0,0,0}})); // 0
        out.println(s.solution(new int[][] {{0,0,0},{1,1,1},{0,1,0}})); // 0
        out.println(s.solution(new int[][] {{0,1,0},{1,1,1},{0,0,0}})); // 1
        out.println(s.solution(new int[][] {{1,0,2,0},{1,2,2,2},{1,1,0,0},{0,0,0,0}})); //2
    }
}